...
class KModel(torch.nn.Module):
    ...
    def _build_text_inputs(
        ...
        text_mask = torch.arange(input_lengths.max(), device=input_ids.device).unsqueeze(0).expand(input_lengths.shape[0], -1)
    ...
    def _predict_alignment(
        self,
        d_en: torch.FloatTensor,
        ref_s: torch.FloatTensor,
        input_lengths: torch.LongTensor,
        text_mask: torch.BoolTensor,
        speed: float = 1
    ) -> tuple[torch.FloatTensor, torch.LongTensor, torch.FloatTensor, torch.FloatTensor]:
        s = ref_s[:, 128:]
        ...
        indices = torch.repeat_interleave(torch.arange(d_en.shape[-1], device=self.device), pred_dur)
        pred_aln_trg = torch.zeros((d_en.shape[-1], indices.shape[0]), device=self.device)
        pred_aln_trg[indices, torch.arange(indices.shape[0])] = 1
        pred_aln_trg = pred_aln_trg.unsqueeze(0).to(self.device)
        en = d.transpose(-1, -2) @ pred_aln_trg
        return d, pred_dur, pred_aln_trg, en

    def _decode_acoustic_tokens(
        self,
        input_ids: torch.LongTensor,
    ...
    def forward_with_tokens(
        ...
        d_en, input_lengths, text_mask = self._encode_linguistic_tokens(input_ids)
        _, pred_dur, pred_aln_trg, en = self._predict_alignment(d_en, ref_s, input_lengths, text_mask, speed)
        audio, _, _ = self._decode_acoustic_tokens(input_ids, pred_aln_trg, en, ref_s)
...
class KEncoderForONNX(torch.nn.Module):
    def __init__(self, kmodel: KModel):
        super().__init__()
        self.kmodel = kmodel
...
class KStaticEncoderForONNX(torch.nn.Module):
    ...
    def forward(
        ...
        d_en = self.kmodel._encode_linguistic_tokens_with_lengths(input_ids, input_lengths, text_mask)
        return d_en.masked_fill(text_mask.unsqueeze(1), 0.0)
...
class KDurationPredictorForONNX(torch.nn.Module):
    ...
    def forward(
        ...
        return self.kmodel._predict_alignment(d_en, ref_s, input_lengths, text_mask, speed)


class KF0NPredictorForONNX(torch.nn.Module):
    def __init__(self, kmodel: KModel):
        super().__init__()
        self.kmodel = kmodel
...
class KTextEncoderForONNX(torch.nn.Module):
    ...
    def forward(
        ...
        input_lengths, text_mask = self.kmodel._build_text_inputs(input_ids)
...
class KStaticTextEncoderForONNX(torch.nn.Module):
    ...
    def forward(
        ...
        t_en = text_encoder.embedding(input_ids)
        t_en = t_en.transpose(1, 2)
        valid_mask = text_mask.unsqueeze(1)
        t_en = t_en.masked_fill(valid_mask, 0.0)
...
class KDecoderForONNX(torch.nn.Module):
    ...
    def forward(
        ...
        return self.kmodel.decoder(asr, F0_pred, N_pred, ref_s[:, :128]).squeeze()
